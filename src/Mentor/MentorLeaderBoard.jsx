...
const MentorLeaderBoard = () => {
  ...
  // Extract unique batches from scheduleData
  const batches = Array.from(
    new Set(scheduleData.flatMap((item) => item.batchNo))
  );
  ...
  useEffect(() => {
    if (activeTab === "Overall") {
      fetchData("Overall");
    } else if (activeTab === "Class" && selectedBatch) {
      fetchData("Class", selectedBatch);
    }
  }, [activeTab, selectedBatch, fetchData]);

  // Set default batch based on available batches
  useEffect(() => {
    if (activeTab === "Class" && batches.length > 0 && !selectedBatch) {
    ...
    <div className="flex flex-col items-center justify-start min-h-screen font-[Inter] text-white mt-16">
      ...
      {activeTab === "Class" && (
        <div className="w-full sm:w-[90%] md:w-[400px] lg:w-[450px] mb-6">
          {batches.length === 0 ? (
            <p className="text-center text-lg font-semibold text-[#2333CB]">
              No batches available
            </p>
          ) : batches.length === 1 ? (
            <p className="text-center text-base font-medium text-[#2333CB] bg-white rounded-full px-4 py-2">
              Batch: {batches[0]}
            </p>
          ) : (
