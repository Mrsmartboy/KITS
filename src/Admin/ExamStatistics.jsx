...
const ExamStatistics = () => {
  ...
  useEffect(() => {
    ...
    if (Location !== 'all' && ['hyderabad', 'bangalore', 'vijayawada'].includes(Location)) {
      filteredLocationStats = {
        [Location]: data.location_stats[Location] || { allocated: 0, attempted: 0, non_attempted: 0 },
      };
      filteredBatches = data.batches.filter(batch => batch.location.toLowerCase() === Location);
    }
    ...
    if (locationFilter) {
      filteredBatches = filteredBatches.filter(batch => batch.location.toLowerCase() === locationFilter.toLowerCase());
    }
      ...
      <div className="w-full p-6 sm:p-4 md:p-6 lg:p-8 font-[Inter]">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg text-center">
          ...
          <p className="text-gray-600">
            No exam statistics available for the selected date{Location !== 'all' ? ` in ${Location}` : ''}.
          </p>
        </div>
  ...
  // Render performance card for a specific location
  const renderLocationPerformance = (loc, index) => {
    ...
    return (
      ...
      >
        <div className="mb-4">
          <span className="text-[#333333] text-base sm:text-lg font-medium">Overall Performance - {loc.charAt(0).toUpperCase() + loc.slice(1)}</span>
  ...
  return (
    <div className="w-full p-6 sm:p-4 md:p-6 lg:p-8 font-[Inter] ">
      ...
      {/* Exam Reports Cards */}
      <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-[40%_58.5%] gap-4 sm:gap-6">
        {/* Overall Performance (shown only when Location is 'all') */}
        {Location === 'all' && (
        ...
        <div className={`w-full lg:border rounded-md lg:shadow-lg bg-[#FFFFFF] md:grid ${Location === 'all' ? 'md:grid-cols-2 lg:grid-cols-2' : 'md:grid-cols-1 lg:grid-cols-1'}`}>
          {Location === 'all'
            ? ['hyderabad', 'vijayawada'].map((loc, index) => renderLocationPerformance(loc, index))
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 w-full sm:w-auto">
            ...
            >
              ...
              {[...new Set(
                (Location === 'all' 
                  ? allBatches 
                  : allBatches.filter(batch => batch.location.toLowerCase() === Location))
                .map(item => item.batch)
              )].map((batch) => (
