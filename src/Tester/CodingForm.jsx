...
const CodingForm = () => {
  ...
  const handleTagNavigation = (tag) => {
    if (tag) {
      navigate(`/upload/coding?subject=${encodeURIComponent(subjectParam)}&tags=${encodeURIComponent(tag)}`);
    } else {
  ...
  const handleCompile = async () => {
    ...
    if (inputs.length === 0) {
      toast.error("At least one input (sample or hidden test case) is required to compile.");
      setIsCompiling(false);
      return;
    }
    ...
    try {
      ...
      if (status === "success" && Array.isArray(results)) {
        ...
        toast.error(`Compilation failed: ${errors || message || "Unknown error"}`, {
          autoClose: 4000,
        });
      } else {
      ...
      toast.error(
        error?.response?.data?.message || error.message || "Failed to compile code. Please try again.",
        {
          autoClose: 4000,
        }
      );
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (!compiledOutputs.sample || compiledOutputs.sample === "No output") {
        toast.error("Please compile the code to generate outputs before submitting.");
      ...
      if (data.hiddenCases && data.hiddenCases.length > 0) {
        for (let idx = 0; idx < data.hiddenCases.length; idx++) {
          if (!compiledOutputs.hidden[idx] || compiledOutputs.hidden[idx] === "No output") {
            toast.error(
      ...
      data.hiddenCases.forEach((tc, idx) => {
        ...
        questionObj[`Hidden_Test_case_${i}_Output`] = compiledOutputs.hidden[idx];
      });
      ...
      toast.success(response.data.message || "Question submitted successfully", {
        autoClose: 2000,
      });
      toast.info(`Total Questions Created on ${tagsParam}: ${response.data.codeCreatedForTag || 0}`, {
        autoClose: 2000,
      });
      ...
      toast.error(
        err?.response?.data?.message || err.message || "Failed to submit question. Please try again.",
        {
          autoClose: 4000,
        }
      );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              ...
              />
              {errors.sourceCode && (
                <p className="text-red-500 text-sm mt-1">{errors.sourceCode.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {errors.sampleInput && (
                <p className="text-red-500 text-sm mt-1">{errors.sampleInput.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                readOnly
                className="mt-1 w-full p-3 border border-[#00007F] rounded-md h-36 focus:outline-none text-sm sm:text-base bg-gray-100"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {fields.map((field, idx) => (
              <div key={field.id} className="border border-[#00007F] rounded-md p-4">
                <div className="flex items-start space-x-4">
                  <div className="flex-1">
                    ...
                    <textarea
                      {...register(`hiddenCases.${idx}.Input`, { required: "Input required" })}
                      placeholder="Input"
                      className="mt-1 w-full p-3 border border-[#00007F] rounded-md h-36 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                    />
                    {errors.hiddenCases?.[idx]?.Input && (
                      <p className="text-red-500 text-sm mt-1">{errors.hiddenCases[idx].Input.message}</p>
                    )}
                  </div>
                  <button
                    type="button"
                    onClick={() => remove(idx)}
                    className="text-sm sm:text-base font-medium text-red-500 hover:underline self-center mt-8"
                  >
                    Remove
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    ...
                    <pre className="mt-1 w-full p-3 border border-[#00007F] rounded-md bg-gray-100 h-36 overflow-auto text-sm sm:text-base text-gray-700">
                      {JSON.stringify(compiledOutputs.hidden[idx]) || "No output yet"}
                    </pre>
                  </div>
                </div>
              </div>
            ...
            <button
              type="button"
              onClick={() => append({})}
              className="text-blue-600 font-semibold text-sm sm:text-base hover:underline"
            >
          ...
          <div className="space-y-4">
            <button
              ...
              className={`w-full p-3 bg-[#00007F] text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base ${
                isCompiling ? "bg-gray-400 cursor-not-allowed" : "hover:bg-blue-700"
              }`}
            >
              {isCompiling ? (
