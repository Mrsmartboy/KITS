...
const StatCard = ({ label, value, color }) => (
  <div className="rounded-[10px] shadow-md bg-white border" style={{ borderColor: color }}>
    <div
...
const CalendarDay = ({ dayNumber, isAttempted, isUnattempted, dayOfWeekName }) => {
  ...
  return (
    ...
    >
      <span className={`text-[12px] leading-none ${textColor}`}>{dayOfWeekName}</span>
      <span className={`text-[18px] font-medium ${textColor}`}>{dayNumber}</span>
    </div>
  );
};
...
const ExamsDetailsManager = ({ data }) => {
  ...
  if (!data || !data.reports || !Array.isArray(data.reports) || data.reports.length === 0) {
    return (
      <div className="w-full min-h-[60vh] flex flex-col items-center justify-center text-center p-6">
        ...
        />
        <p className="text-xl font-semibold text-gray-600 mb-1">No Exam Details Available</p>
        <p className="text-gray-500 text-sm">Search for a student to see their details here.</p>
      </div>
    );
  }
  ...
  const monthYearHeader = calendarDate.toLocaleDateString('en', { month: 'long', year: 'numeric' });
  ...
  return (
    <>
      <div className="w-full flex justify-center items-center p-4 md:p-6 lg:p-10 font-[inter]">
        <div className="flex flex-col lg:flex-row gap-6 w-full max-w-full">
          <div className="flex-1 bg-white shadow-[0px_4px_20px_#B3BAF7] rounded-lg px-4 sm:px-6 md:px-8 py-6">
            <h2 className="text-[#19216F] text-xl font-bold mb-6">Exams Details Overview</h2>
                ...
                <div className="relative w-full h-[46px] bg-white shadow-md rounded flex items-center px-4">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value) setCalendarDate(new Date(e.target.value));
                    }}
          ...
          <div className="w-full lg:w-[456px] h-auto bg-white shadow-[0px_4px_20px_#B3BAF7] rounded-lg p-4 md:p-6">
            <div className="w-full flex justify-between items-center mb-4">
              <button
                ...
                className="text-[#19216F] hover:text-[#10164b]"
                aria-label="Previous month"
              >
                ←
              </button>
              <h3 className="text-lg font-bold text-gray-700">{monthYearHeader}</h3>
              <button
            ...
            <div className="grid grid-cols-7 gap-[10px]" role="grid" aria-label="Calendar">
              {[...Array(daysInMonth)].map((_, i) => {
                const dayNumber = i + 1;
                const dateObj = new Date(displayedYear, displayedMonth, dayNumber);
                const dayOfWeekName = dateObj.toLocaleDateString('en', { weekday: 'short' });
                return (
                  <CalendarDay
                    key={dayNumber}
                    dayNumber={dayNumber}
      ...
      <div className="w-full flex flex-col items-center font-[inter] py-10 px-4 md:px-8 lg:px-16">
        <h2 className="text-[#19216F] text-[32px] font-semibold uppercase mb-6 text-center">
          Student Exam Details
        </h2>
        <div className="flex flex-col md:flex-row gap-6 mb-10 w-full max-w-[1373px] items-center">
          <div>
            <h3 className="text-[#19216F] font-semibold text-[24px] mb-2">From Date</h3>
            <div className="w-[263px] h-[46px] bg-white shadow-md rounded-[2px] px-4 flex items-center">
              <input
                type="date"
                className="w-full text-[#19216F] text-[16px] outline-none bg-transparent cursor-pointer"
                value={fromDate}
          ...
          <div>
            <h3 className="text-[#19216F] font-semibold text-[24px] mb-2">To Date</h3>
            <div className="w-[263px] h-[46px] bg-white shadow-md rounded-[2px] px-4 flex items-center">
              <input
                type="date"
                className="w-full text-[#19216F] text-[16px] outline-none bg-transparent cursor-pointer"
          ...
          <div className="ml-auto mt-6 md:mt-10 flex gap-4">
            <button
              className="bg-[#19216F] text-white px-6 py-2 rounded-md shadow-md hover:bg-[#10164b] transition-all"
        ...
        <div className="w-full max-w-[1373px] bg-white shadow-[0px_4px_20px_#B3BAF7]">
          <div
            className="w-full bg-[#19216F] text-white flex justify-between items-center px-4 h-[53px] text-[16px] font-semibold"
            role="row"
          >
          ...
          <div className="max-h-[515px] overflow-y-auto custom-scrollbar" role="rowgroup">
            {tableRows.length === 0 ? (
